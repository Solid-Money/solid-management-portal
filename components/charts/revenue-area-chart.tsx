...
function getRevenueDiversification(
  ...
  if (total === 0) return { label: "No data", color: "text-gray-500", icon: "⚪" };

  const maxShare = Math.max(yieldShare, borrowingFees) / total;

  if (maxShare <= 0.4) {
    return { label: "Well Diversified", color: "text-emerald-600", icon: "🟢" };
  }
  if (maxShare <= 0.6) {
    return { label: "Moderately Diversified", color: "text-amber-600", icon: "🟡" };
  }
  return { label: "Concentrated", color: "text-red-600", icon: "🔴" };
}
